...
One of the most important evidences of the beyond SM phenomena is the existence of DM \cite{Zwicky:1933}.
...
When $x \gg 1$, which is the same as the non-relativistic limit, the term with the highest power of $x$ dominates the cross section.  When the $x^{-p}$ term dominates ($p \geq 0$), temperature dependence of the interaction rate is $\Gamma \propto x^{-3/2-p} e^{-x}$, while the Hubble parameter only reduces as $H \propto \rho^{1/2} \propto x^{-2}$.
...
The threshold effect can be take into account by choosing the lower boundary of the $\bm{v}$-integral in Eq.~\eqref{eq:rate} to be $v_{\mathrm{min}}$ defined as
\begin{align}
...
These two behaviors determines the best suitable $m_\chi$ for each choice of $m_T$ and $E_{\mathrm{thr}}$, which is the reason why the xenon nucleus target is more suitable for the $\mathrm{TeV}$-scale WIMP search than the electron target.
...
For example, the calculation for Wino up to the next-to-leading order in $\alpha_s$ reveals that the cross section almost mass-independently takes a small value of $\sigma_p^{\mathrm{SI}} \simeq 2.3 \times 10^{-47}\,\mathrm{cm}^2$~\cite{Hisano:2015rsa}, which is below the current bound but is a region of future interest.
...
For Higgsino-like LSP, the constraint is highly model-dependent since the mixing between the Higgsino and gauginos such as Bino or Wino significantly modifies the scattering cross section.
...
\rem{Comment on non-zero $U(1)_Y$ hypercharge}
