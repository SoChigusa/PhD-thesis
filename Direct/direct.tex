...
Recalling that the detectors are installed in a cyrindrical geometry, the probability for a WIMP to reach the position apart from the beam line by $d_T$ plays an important roll.
We can estimate the probability $P(d_T)$ as
\begin{align}
  P(d_T) = \exp \left( -\frac{d_T}{\beta \gamma c \tau \sin\theta} \right),
\end{align}
...
\begin{figure}[t]
  ...
  \caption{
    ...
    The pair prodction process at $\sqrt{s}=100\,\mathrm{TeV}$ and $\mathcal{L} = 30\,\mathrm{ab}^{-1}$ are assumed.
  }
  \label{fig:beta}
\end{figure}

In Fig.~\ref{fig:beta}, we show the distribution of the Wino velocity $\beta$ for the same process.
Blue histogram shows the distribution of all events
