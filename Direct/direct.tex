...
\begin{figure}[t]
  ...
  \caption{
    Distribution of the survival probability $P(r = 15\,\mathrm{cm})$ for $2.9\,\mathrm{TeV}$ Wino.
    $\sqrt{s}=100,\mathrm{TeV}$ and $\mathcal{L} = 30\,\mathrm{ab}^{-1}$ are assumed.}
  \label{fig:survival_probability}
\end{figure}

From now on, we estimate how many events are expected at the FCC-hh.
Recalling that the detectors are installed in a cyrindrical geometry, the probability for a WIMP to reach the position apart from the beam line by $r$ plays an important roll.
We can estimate the probability $P(r)$ as
\begin{align}
  P(r) = \exp \left( -\frac{r}{\beta \gamma c \tau \sin\theta} \right),
\end{align}
...
In Fig.~\ref{fig:survival_probability}, we show the distribution of $P(r = 15\,\mathrm{cm})$ (which is motivated by the FCC-hh detector setup assumed below), assuming the $2.9\,\mathrm{TeV}$ Wino, $\sqrt{s} = 100\,\mathrm{TeV}$, and $\mathcal{L} = 30\,\mathrm{ab}^{-1}$.
Note that $\tau \simeq 5.8\,\mathrm{cm}$ for this setup.
We can see that the increase in the probability by $\gamma$ and the decrease by $\beta$ and $\sin\theta$ roughly cancels with each other, resulting in a peak of the distribution at $P \sim 10^{-1} \sim \exp (-15\,\mathrm{cm} / \tau)$.
