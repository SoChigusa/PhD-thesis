...
Depneding on the size of the relevant mass diferrence $\Delta m$, there are several channels that contributes to the decay \cite{Chen:1995yu}.
...
So far, the search is performed by both ATLAS \cite{Aaboud:2017mpt} and CMS \cite{Sirunyan:2018ldc} collborations at the LHC.
Below, we will focus particularly on the ATLAS collaboration to explain the detector setup and the current status.

In the ATLAS setup, several tracking detectors are equipped cyrindrically around the beam line from the radius $r = 3\,\mathrm{cm}$ to $108\,\mathrm{cm}$.
...
These two requirements efficiently reduce the contamination from the mis-reconstruction of the pile-up or SM background events and enable us to detect faint signals of WIMPs.
...
In Fig.~\ref{fig:ATLAS_disappearing_track}, we show the current status of the disappearing track search at the ATLAS collaboration \cite{Aaboud:2017mpt}.
