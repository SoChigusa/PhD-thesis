...
\end{align}
\rem{Check!  Derive MSbar formula?}
...
\footnote{
  ...
  However, the parameter space with $\tilde{X}_t \gtrsim 6$ is severely constrained by the requirement of the stability of the electroweak vacuum \rem{Citation} and is not considered here.
}
...
As we will see in Sec.~\rem{????}, the light Higgsino is also fascinating as a dark matter candidate.
...
We will return to this point and check that this is the case in Sec.~\rem{???}, related to the collider search for MDMs using the disappearing track signal.
...
Since the well-motivated DM mass is of $\mathcal{O} (\mathrm{TeV})$ as we will see in Sec.~\rem{??}, the resulting lifetime of the DM candidate is estimated as $\tau \sim \Lambda^{-2p} (\mathrm{TeV})^{2p-1}$ for an operator with a suppression factor $\Lambda^{-p}$.
...
Since the non-zero value of $Y$ usually leads to the large cross section as will be discussed in Sec.~\rem{???}, only $Y=0$ candidates are associated with checkmarks.
...
\begin{table}
  ...
  \caption{
    ...
    See Sec.~\rem{???} for the descriptions and implications of $m_\chi$ and Sec.~\ref{sec:mass_splitting} for those of $\Delta m_\chi$.
  }
...
$m_\chi$ is the required masses to explain the DM relic abundance without non-thermal production (see Sec.~\rem{???} for the detail).
