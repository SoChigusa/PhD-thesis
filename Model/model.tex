...
\footnote{
  ...
  However, the parameter space with $\tilde{X}_t \gtrsim 6$ is severely constrained by the requirement of the stability of the electroweak vacuum \rem{Reference!!} and is not considered here.
}
...
We will return to this point and check that this is the case in Sec.~??? \rem{Caution!!}, related to the collider search for MDMs using the disappering track signal.
...
Since the well-motivated DM mass is of $\mathcal{O} (\mathrm{TeV})$ as we will see in Sec.~?? \rem{Caution!!}, the resulting lifetime of the DM candidate is estimated as $\tau \sim \Lambda^{-2p} (\mathrm{TeV})^{2p-1}$ for an operator with a suppression factor $\Lambda^{-p}$.
...
Since the non-zero value of $Y$ usually leads to the large cross section as will be discussed in Sec.~??? \rem{Caution!!}, only $Y=0$ candidates are associated with checkmarks.
\rem{Correct? in particular for $5$- and $7$-plets?  References??}
