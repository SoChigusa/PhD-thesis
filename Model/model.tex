...
Even when the SUSY breaking scale is much higher than the electroweak scale, it is natural for Higgsino to be around the electroweak scale since it is protected by an R-symmetry and a Peccei Quinn symmetry.  \rem{Reference!!}

\rem{Model with small $\mu$??}
This naturalness requirement also impose an upper bound on other parameters, in particular on $m_{H_u}^2$ for $\tan^2 \beta \gg 1$.
...
The situation is similar for Higgsino, since it is easy to realize the hidden sector dynamics that generates the $\mu$-term of $\mathcal{O} (m_{3/2})$.
